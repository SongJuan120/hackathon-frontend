...
const WalletConnet = () => {
  ...
  const { connectWallet, wallet } = useMessageForSignature();

  useEffect(() => {
    if (wallet) { 
      dispatch(login(wallet.address));
    };
  }, [wallet])
  ...
  const onCoinBase = async() =>{
  
  }
  
  const onWalletConnet = async() =>{
  ...
  return (
    <StyledPage>
      <div tw="mx-auto max-w-screen-sm px-3 pt-2">
        ...
        <div tw="mt-5">
          <div onClick={onMetaMask} tw="border border-solid rounded-lg border-zinc-200 px-7 py-3 mb-3 cursor-pointer hover:bg-[#FBF8FB]">
            <div tw="flex items-center">
              <img alt="metamask" src={imgMetaMask} tw="w-[32px]"/>
              <div tw="text-base font-semibold text-black ml-3">Metamask</div>
            </div>
          </div>
          <div onClick={onCoinBase} tw="border border-solid rounded-lg border-zinc-200 px-7 py-3 mb-3 cursor-pointer hover:bg-[#FBF8FB]">
            <div tw="flex items-center">
              <img alt="coinbase" src={imgCoinBase} tw="w-[32px]"/>
              <div tw="text-base font-semibold text-black ml-3">Coinbase Wallet</div>
            </div>
          </div>
          <div onClick={onWalletConnet} tw="border border-solid rounded-lg border-zinc-200 px-7 py-3 cursor-pointer hover:bg-[#FBF8FB]">
            <div tw="flex items-center">
